 ...
 *       (50 % by default). Below that threshold the plan change is downgraded to a
 *       {@code BASELINE_EXCEEDED} warning: the query is still well within its SLA
 *       budget, so the nightly benchmark should confirm the impact at scale.</li>
    ...
    private void checkPlanChange(String queryId,
        ...
        if (slaRiskPct >= thresholdPct) {
            ...
            String description = nodeChanged
                    ? String.format(
                            "[%s] plan changed: '%s' → '%s' (cost %.2f → %.2f, +%d%%); " +
            ...
            String description = nodeChanged
                    ? String.format(
                            "[%s] plan changed: '%s' → '%s' (cost %.2f → %.2f, +%d%%) — " +
                            "warning only (p95=%.0f ms is %.0f%% of SLA; nightly will confirm at scale)",
                            queryId, baselineNode, currentNode,
                            baseCost, current.getPlanCost(), (int)(planCostTolerancePct * 100),
                            current.getP95Ms(), slaRiskPct)
                    : String.format(
                            "[%s] plan cost=%.2f increased +%d%% vs baseline — " +
                            "warning only (p95=%.0f ms is %.0f%% of SLA; nightly will confirm at scale)",
                            queryId, current.getPlanCost(), (int)(planCostTolerancePct * 100),
                            current.getP95Ms(), slaRiskPct);
